...
function animatePlanets() {
    ...
    planetData.forEach(planet => {
        ...
        const t = 1 - (planet.radius - minRadius) / (maxRadius - minRadius);
        const lightStrength = 0.40 + t * 0.50;  // INCREASED: range ~0.40–0.90 (was 0.20-0.60)
...
function goToGalaxy() {
    printLine('🌌 Opening Galaxy Explorer...', 'yellow');
    printLine('Redirecting to galaxy page...', 'cyan');
    setTimeout(() => {
...
function showHelp() {
    ...
    printLine('║  /galaxy     →  🌌 Explore 3D Galaxy!              ║', 'cyan');
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        ...
        else if (document.getElementById('galaxy-page').classList.contains('active')) {
            if (window.stopGalaxyAnimation) window.stopGalaxyAnimation();
            goHome();
        }
    }
});